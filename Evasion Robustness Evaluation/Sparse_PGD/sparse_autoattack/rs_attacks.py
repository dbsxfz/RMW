...
class RSAttack():
    ...
    def get_init_patch(self, c, s, n_iter=1000):
        ...
        if self.init_pattern is not None:
            patch_univ = patch_univ / 16 + self.init_pattern
    ...
    def attack_single_run(self, x, y):
        with torch.no_grad():
            ...
            if self.norm == 'L0':
                ...
                x_best = x.clone()
                n_pixels = h * w
                b_all, be_all = torch.zeros([x.shape[0], eps], device=x.device).long(), torch.zeros([x.shape[0], n_pixels - eps], device=x.device).long()
